...
Utility to find AES keys in running process memory. Works for 128, 192 and 256-bit keys and you can also find keys from memory dump files.

# Usage
```
g++ aes-finder.cpp -o aes-finder.exe
```

find keys by process id
```
./aes-finder.exe -p 123
```

find keys by process name
```
./aes-finder.exe -n chrome.exe
```

find keys by memory dump file
```
